...
class EnhancedActivationVisualizer:
    ...
    def get_network_activations(self, z, action=None, next_z=None):
        ...
        with torch.no_grad():
            ...
            if action is not None and next_z is not None:
                ...
                if padding_size > 0:
                    forward_hidden = torch.nn.functional.pad(
                        forward_hidden, (0, padding_size)
                    )
            else:
            ...
            if next_z is not None:
                ...
                if padding_size > 0:
                    inverse_hidden = torch.nn.functional.pad(
                        inverse_hidden, (0, padding_size)
                    )
            else:
    ...
    def setup_visualization(self):
        ...
        self.forward_pca.fit(
            np.vstack(collections["forward"])
        )  # Forward data should be 64-dim
        self.inverse_pca.fit(
            np.vstack(collections["inverse"])
        )  # Inverse data should be 64-dim
    ...
    def update_visualization(
        self, obs, prev_obs=None, prev_action=None, episode_return=0
    ):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if prev_obs is not None:
                    prev_mu, prev_logvar = self.agent.upn.encode(
                        torch.Tensor(prev_obs).to(self.device)
                    )
                ...
                if prev_z is not None and prev_action is not None:
                    activations = self.get_network_activations(
                        prev_z, torch.Tensor(prev_action).to(self.device), z
                    )
                else:
                    activations = self.get_network_activations(z)
                ...
                current_points = {
                    ...
                    "forward": self.forward_pca.transform(
                        activations["forward"].cpu().numpy()
                    ),
                    "inverse": self.inverse_pca.transform(
                        activations["inverse"].cpu().numpy()
                    ),
                }
    ...
    def run_episode(self):
        ...
        while not next_done.all():
            self.update_visualization(
                next_obs, prev_obs, prev_action, episode_return.item()
            )
...
def main():
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env_with_render(
                args_test.env_id, 0, True, args_test.exp_name, args_test.gamma
            )
        ]
    )
    print(f"Environment created: {args_test.env_id}")

    agent = SOFAgent(envs).to(device)
    print(agent)
    model_path = os.path.join(
        os.getcwd(), "sof", "params", "sofppo", args_test.sof_path
    )
