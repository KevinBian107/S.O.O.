...
class Agent_sof(nn.Module):
    def __init__(self, envs):
        ...
        self.eta_k = torch.ones((args_sof.num_envs,), requires_grad=True).to(
            args_sof.device
        )
    ...
    def load_upn(self, file_path):
        ...
        if os.path.exists(file_path):
            ...
            print(
                f"No existing UPN model found at {file_path}, starting with new parameters."
            )
    ...
    def load_ppo(self, file_path):
        ...
        if os.path.exists(file_path):
            ...
            ppo_state_dict = {
                ...
                if "actor_mean" in k or "critic" in k or "actor_logstd" in k
            }

            self.load_state_dict(ppo_state_dict, strict=False)
        else:
            print(
                f"No existing PPO model found at {file_path}, starting with new parameters."
            )
...
class Agent_ppo(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_mean = nn.Sequential(
            ...
            layer_init(
                nn.Linear(
                    args_ppo.ppo_hidden_layer, np.prod(envs.single_action_space.shape)
                ),
                std=0.01,
            ),
        )
