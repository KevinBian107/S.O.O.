    ...
    for episode in range(num_episodes):
        ...
        while not next_done.all():
            with torch.no_grad():
                ...
                print(
                    f"Episode: {episode}/{num_episodes} With Rewards: {episode_returns}"
                )
...
if __name__ == "__main__":
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env(
                args_test.env_id,
                i,
                args_test.capture_video,
                args_test.exp_name,
                args_test.gamma,
            )
            for i in range(args_test.num_envs)
        ]
    )

    # Load the SOF-PPO model
    sfmppo_agent = SOFAgent(envs).to(device)
    sfmppo_path = os.path.join(
    ...
    sfmppo_returns = evaluate_model(
        sfmppo_agent, envs, device, num_episodes=episode_num
    )
    ppo_returns = evaluate_model(ppo_agent, envs, device, num_episodes=episode_num)

    plt.figure(figsize=(10, 6))
    plt.plot(
        range(1, len(sfmppo_returns) + 1), sfmppo_returns, label="SOF-PPO", marker="o"
    )
    plt.plot(range(1, len(ppo_returns) + 1), ppo_returns, label="PPO", marker="o")
    plt.title(
        "Episode Returns for Intention Constrain Models Evaluated in Intention Environment"
    )
    plt.xlabel("Episode")
    plt.ylabel("Return")
    plt.legend()
