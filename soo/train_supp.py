...
def train_model(model, dataloader, optimizer):
    ...
    for states, actions, next_states in dataloader:
        ...
        recon_loss, forward_loss, inverse_loss, consistency_loss = compute_upn_loss(
            model, states, actions, next_states
        )
    ...
    return (
        ...
        total_forward_loss / len(dataloader),
        total_inverse_loss / len(dataloader),
...
def train_supp():
    ...
    train_dataloader = DataLoader(
        train_dataset, batch_size=args_supp.batch_size, shuffle=True
    )
    val_dataloader = DataLoader(
        val_dataset, batch_size=args_supp.batch_size, shuffle=False
    )
    ...
    optimizer = optim.Adam(
        model.parameters(), lr=args_supp.learning_rate, weight_decay=1e-5
    )
    ...
    for epoch in range(args_supp.num_epochs):
        ...
        print(
            f"Train - Total: {train_loss[0]:.4f}, Recon: {train_loss[1]:.4f}, Forward: {train_loss[2]:.4f}, Inverse: {train_loss[3]:.4f}, Consistency: {train_loss[4]:.4f}"
        )
        print(
            f"Val   - Total: {val_loss[0]:.4f}, Recon: {val_loss[1]:.4f}, Forward: {val_loss[2]:.4f}, Inverse: {val_loss[3]:.4f}, Consistency: {val_loss[4]:.4f}"
        )
