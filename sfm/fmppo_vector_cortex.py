...
class Args:
    ...
    expert_weight_start: float = 1.0  
    expert_weight_end: float = 0.0    
    expert_weight_decay_steps: int =  200000/2
...
class UPN(nn.Module):
    def __init__(self, state_dim, action_dim, latent_dim):
        ...
        self.inverse_dynamics_logstd = nn.Parameter(torch.zeros(1, action_dim))
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        z = self.upn.encoder(x) # this gives NaN with big latent
        ...
        optimal_next_z = self.upn.dynamics(z)
        expert_advised_action_mean = self.upn.inverse_dynamics(torch.cat([z, optimal_next_z], dim=-1))
        expert_advised_action_logstd = self.upn.inverse_dynamics_logstd.expand_as(expert_advised_action_mean)
        expert_advised_action_std = torch.exp(expert_advised_action_logstd)
        expert_advised_probs = Normal(expert_advised_action_mean, expert_advised_action_std)
        expert_advised_action = expert_advised_probs.sample()
        
        if global_step < args.expert_weight_decay_steps:
            final_action = expert_advised_action
            probs = expert_advised_probs
        else:
            final_action = action
            probs = probs

        # step_frac = min(global_step / args.expert_weight_decay_steps, 1.0)
        # expert_weight = args.expert_weight_start * (1.0 - step_frac) + args.expert_weight_end * step_frac

        # corrective_action = expert_advised_action - action
        # final_action = action + expert_weight * corrective_action

        return final_action, probs.log_prob(final_action).sum(1), probs.entropy().sum(1), self.critic(z)
