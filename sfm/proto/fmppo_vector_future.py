...
class Args:
    ...
    upn_coef: float = 1.0
    load_upn: str = None #"fm_vector.pth"
...
class UPN(nn.Module):
    ...
    def forward(self, state, action, next_state, future_states):
        z = self.encoder(state)
        z_next = self.encoder(next_state)
        z_future = [self.encoder(fs) for fs in future_states]

        # Multi-step prediction with same action (consider different action?)
        z_pred = self.dynamics(torch.cat([z, action], dim=-1))
        z_pred_future = [z_pred]
        for _ in range(len(future_states) - 1):
            z_pred = self.dynamics(torch.cat([z_pred, action], dim=-1))
            z_pred_future.append(z_pred)

        action_pred = self.inverse_dynamics(torch.cat([z, z_next], dim=-1))

        # For consistency loss
        z_consistent = self.dynamics(torch.cat([z, action_pred], dim=-1))

        state_recon = self.decoder(z)
        next_state_pred = self.decoder(z_pred)
        next_state_recon = self.decoder(z_next)
        future_states_pred = [self.decoder(z_f) for z_f in z_pred_future]

        return z, z_next, z_pred, z_consistent, action_pred, state_recon, next_state_recon, next_state_pred, future_states_pred, z_future
...
def compute_upn_loss(upn, states, actions, next_states, lambda_consistency=0, lambda_multi_step=0, lambda_temporal_coherence=0):
    '''For multi-step prediction, we'll use the next few states in the batch as future states, use up to 4 future states'''

    batch_size = states.shape[0]
    future_states = next_states[1:min(batch_size, 5)]

    z, z_next, z_pred, z_consistent, action_pred, state_recon, next_state_recon, next_state_pred, future_states_pred, z_future = upn(states, actions, next_states, future_states)

    recon_loss = F.mse_loss(state_recon, states) + F.mse_loss(next_state_recon, next_states)
    forward_loss = F.mse_loss(z_pred, z_next.detach())
    inverse_loss = F.mse_loss(action_pred, actions)

    # Consistency loss
    consistency_loss = F.mse_loss(z_consistent, z_next.detach())

    # Multi-step prediction loss
    # multi_step_loss = sum(F.mse_loss(fs_pred, fs.detach()) for fs_pred, fs in zip(future_states_pred, future_states.unbind(1)))
    multi_step_loss = sum(F.mse_loss(fs_pred, fs.detach()) for fs_pred, fs in zip(future_states_pred, future_states))

    # Temporal coherence loss
    temporal_coherence_loss = sum(F.mse_loss(z1, z2) for z1, z2 in zip(z_future[:-1], z_future[1:]))
    ...
    upn_loss = (
        ...
        + lambda_multi_step * multi_step_loss
        + lambda_temporal_coherence * temporal_coherence_loss
    )
...
def prepare_future_states(obs, next_obs, num_future_steps):
    batch_size, obs_dim = obs.shape
    future_states = torch.zeros((batch_size, num_future_steps, obs_dim), device=obs.device)
    
    # First future state is always the next_obs
    future_states[:, 0] = next_obs

    # For remaining future steps, use a rolling window approach
    for i in range(1, num_future_steps):
        future_states[:, i] = obs.roll(-i, dims=0)
        
        # Replace the last i rows with the last available state
        future_states[-i:, i] = obs[-1]
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        # future_states = prepare_future_states(b_obs_imitate, b_next_obs_imitate, num_future_steps=args.num_future_steps)
    ...
    data_filename = f"fmppo_vector_latent_test.pth"
    data_path = os.path.join(save_dir, data_filename)

    data_filename = f"fm_vector_latent_test.pth"
