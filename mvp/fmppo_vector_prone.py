...
from env_wrappers import (JumpRewardWrapper, TargetVelocityWrapper, DelayedRewardWrapper, MultiTimescaleWrapper, 
                          NoisyObservationWrapper, PartialObservabilityWrapper, ActionMaskingWrapper)
...
class UPN(nn.Module):
    ...
    def forward(self, state, action, next_state):
        ...
        state_next_recon = self.decoder(z_next)
        return z, z_next, state_recon, state_next_recon
...
def compute_upn_loss(upn, state, action, next_state):
    z, z_next, state_recon, next_state_recon = upn(state, action, next_state)
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig('fmppo_prone_results.png')
    plt.show()
