...
def compute_supp_upn_loss(upn, state, action, next_state):
    ...
    z, z_next, z_pred, action_pred, state_recon, next_state_recon, next_state_pred = (
        upn(state, action, next_state)
    )
    recon_loss = nn.MSELoss()(state_recon, state) + nn.MSELoss()(
    ...
    latent_regularization = torch.mean(torch.norm(z, p=2, dim=-1)) + torch.mean(
        torch.norm(z_next, p=2, dim=-1)
    )
...
def compute_upn_loss(upn, state, action, next_state):
    ...
    recon_loss = F.mse_loss(state_recon, state) + F.mse_loss(
        next_state_recon, next_state
    )
...
def optimize_eta_k(states, advantages, old_policy, epsilon_k):
    ...
    result = minimize(
        eta_k_objective,
        [eta_initial],
        args=(states, advantages, old_policy, epsilon_k),
        bounds=[(1e-3, None)],
        method="L-BFGS-B",
...
def mixed_batch(ppo_states, ppo_actions, ppo_next_states):
    ...
    imitation_next_states = torch.FloatTensor(imitation_data["next_states"]).to(
        args_sof.device
    )
