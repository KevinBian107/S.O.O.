...
def optimize_model(average_outer_products, count_gradients):
    '''optimization steps'''
    ...
    action_batch = torch.tensor([t.action for t in transitions], device=device, dtype=torch.float)
    action_batch = action_batch.type(torch.int64).unsqueeze(1) # need to match 2d tensor
    ...
    # Compute and store the gradient outer product for each parameter
...
def main(checkpoint_path=None):
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            observation, reward, terminated, truncated, _ = env.step([actual_action]) # pass in as a 1d
            ...
            optimize_model(average_outer_products, count_gradients)
